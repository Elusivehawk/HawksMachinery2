...
public class HMItemElectricTool extends HMItemElectric
{
	...
	@Override
	public boolean onBlockDestroyed(ItemStack item, World world, int blockID,
			int x, int y, int z, EntityLiving entity)
	{
		if (ForgeHooks.canToolHarvestBlock(Block.blocksList[blockID],
				world.getBlockMetadata(x, y, z), item))
		{
			this.onUse(250, item);
	...
	{
		return ForgeHooks.canToolHarvestBlock(block, meta, item) ? this.toolSpeed
				: 1.0F;
	}
