...
import com.elusivehawk.mcmods.hm1.machine.HMInventoryCrafting;
	...
	public void onCreate(Vector3 placedPosition)
	{
		for (int x = -1; x < 2; ++x)
		{
			for (int z = -1; z < 2; ++z)
			{
				if (x != 0 || z != 0)
				{
					((BlockMulti)HMCore.technicalBlock).makeFakeBlock(this.worldObj, new Vector3(this.xCoord + x, this.yCoord, this.zCoord + z), placedPosition);
